...
class FSDPSFTTrainer(object):
    ...
    def _build_dataloader(self):
        ...
        self.train_dataset = SFTDataset(parquet_files=config.data.train_files,
                                        ...
                                        filter_overlong_prompts=config.data.get('filter_overlong_prompts', True),
        ...
        self.val_dataset = SFTDataset(parquet_files=config.data.val_files,
                                      ...
                                      filter_overlong_prompts=config.data.get('filter_overlong_prompts', True),
                                      apply_chat_template=config.data.get('apply_chat_template', True))
        # randomly subsample validation dataset if needed using pytorch
        if self.config.data.val_subsample_ratio is not None:
            self.val_dataset = Subset(self.val_dataset, torch.randperm(len(self.val_dataset))[:int(len(self.val_dataset) * self.config.data.val_subsample_ratio)])
        # build dataloader
    ...
    def _build_model_optimizer(self):
        ...
        if self.config.trainer.total_training_steps is not None:
            ...
            if self.device_mesh.get_rank() == 0:
                print(f'Total training steps: {self.total_steps} (overwrite epochs)')
        else:
            if self.device_mesh.get_rank() == 0:
                print(
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(self.train_dataloader,
                ...
                if (self.config.trainer.test_freq > 0 and (is_last_step or \
                    global_step % self.config.trainer.test_freq == 0)) or global_step == 1:
