...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self.ulysses_sequence_parallel_size > 1:
            self.ulysses_device_mesh = init_device_mesh('cuda',
                                                        mesh_shape=(
                                                            dp, self.ulysses_sequence_parallel_size),
        ...
        self.role = role
        assert self.role in ['actor', 'rollout', 'ref',
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor' and optim_config is not None:
            from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_linear_schedule_with_warmup, get_exponential_schedule_with_warmup
            actor_optimizer = optim.AdamW(actor_module_fsdp.parameters(),
                                          lr=optim_config.lr,
                                          betas=optim_config.get(
                                              'betas', (0.9, 0.999)),
            ...
            if optim_config.get('lr_scheduler', 'constant') == 'linear':
                actor_lr_scheduler = get_linear_schedule_with_warmup(optimizer=actor_optimizer,
                                                                     num_warmup_steps=num_warmup_steps,
                                                                     num_training_steps=total_steps,
                                                                     min_lr_ratio=optim_config.get('min_lr_ratio', 0.1),
                                                                     decay_start_ratio=optim_config.get('decay_start_ratio', 0.0))
            elif optim_config.get('lr_scheduler', 'constant') == 'exponential':
                actor_lr_scheduler = get_exponential_schedule_with_warmup(optimizer=actor_optimizer,
                                                                         num_warmup_steps=num_warmup_steps,
                                                                         num_training_steps=total_steps,
min_lr_ratio=optim_config.get('min_lr_ratio', 0.1),
                                                                     decay_start_ratio=optim_config.get('decay_start_ratio', 0.0))
    ...
    def _build_rollout(self):
        ...
        rollout_device_mesh = init_device_mesh('cuda', mesh_shape=(
            dp, infer_tp), mesh_dim_names=['dp', 'infer_tp'])
        ...
        if rollout_name == 'hf':
            ...
            log_gpu_memory_usage(
                f'Before building {rollout_name} rollout', logger=None)
            ...
            log_gpu_memory_usage(
                f'After building {rollout_name} rollout', logger=None)
            ...
            rollout_sharding_manager = FSDPVLLMShardingManager(module=self.actor_module_fsdp,
                                                               ...
                                                               device_mesh=rollout_device_mesh)
            log_gpu_memory_usage(
                'After building sharding manager', logger=None)
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics['perf/max_memory_allocated_gb'] = torch.cuda.max_memory_allocated() / \
                (1024**3)
            metrics['perf/max_memory_reserved_gb'] = torch.cuda.max_memory_reserved() / \
                (1024**3)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_linear_schedule_with_warmup, get_exponential_schedule_with_warmup
        if config.optim.get('lr_scheduler', 'constant') == 'linear':
            critic_lr_scheduler = get_linear_schedule_with_warmup(optimizer=critic_optimizer,
                                                                  num_warmup_steps=num_warmup_steps,
                                                                  num_training_steps=total_steps,
                                                                  min_lr_ratio=config.optim.get('min_lr_ratio', 0.1),
                                                                  decay_start_ratio=config.optim.get('decay_start_ratio', 0.0))
        elif config.optim.get('lr_scheduler', 'constant') == 'exponential':
            critic_lr_scheduler = get_exponential_schedule_with_warmup(optimizer=critic_optimizer,
                                                                       num_warmup_steps=num_warmup_steps,
                                                                       num_training_steps=total_steps,
                                                                       min_lr_ratio=config.optim.get('min_lr_ratio', 0.1),
                                                                  decay_start_ratio=config.optim.get('decay_start_ratio', 0.0))
    ...
    def update_critic(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            metrics['perf/mfu/critic'] = estimated_flops * \
                self.config.ppo_epochs / promised_flops / self.world_size
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # pad it back
                rm_score = pad_input(
                    reward_rmpad, indices=indices, batch=batch_size, seqlen=seqlen).squeeze(-1)
            else:
    ...
    def _expand_to_token_level(self, data: DataProto, scores: torch.Tensor):
        ...
        token_level_scores = torch.zeros_like(
            attention_mask, dtype=scores.dtype)  # (bsz, seqlen)
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        rm_inputs = {'input_ids': rm_input_ids,
                     'attention_mask': rm_attention_mask, 'position_ids': rm_position_ids}
