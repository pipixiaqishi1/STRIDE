...
def get_linear_schedule_with_warmup(
    ...
    assert 0.0 <= decay_start_ratio <= 1.0, "decay_start_ratio must be in [0, 1]"

    decay_start_step = int(num_training_steps * decay_start_ratio)

    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            # Warmup: linearly increase LR from 0 to 1
            return float(current_step) / float(max(1, num_warmup_steps))
            ...
            # Linear decay starts from decay_start_step
            decay_progress = float(current_step - decay_start_step) / float(max(1, num_training_steps - decay_start_step))
...
def get_exponential_schedule_with_warmup(
    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            ...
            # Exponential decay from 1.0 to min_lr_ratio
            ...
            # Exponential decay function: lr = exp(-k * progress), scaled to [min_lr_ratio, 1.0]
            decay_factor = math.exp(-5 * decay_progress)  # -5 controls the steepness
            return max(min_lr_ratio, min_lr_ratio + (1.0 - min_lr_ratio) * decay_factor)
